...
class Closure():
    ...
    The Closure class is designed to be used with different optimization methods, 
    including Particle Swarm Optimization (PSO), Constrained Stochastic Optimization (CSO), 
    Neural Gradient Descent (NGD), and Neural Network Conjugate Gradient (NNCG). 
    It provides a common interface for these methods and handles the underlying 
    computations, including loss calculation, backpropagation, and model updates.

    Methods:
        __init__(mixed_precision, model): Initializes the object with the given mixed precision and model.
        set_model(model): Sets the model for the current instance.
    ...
    def _closure(self):
        """
    Performs a single optimization step by computing the loss, applying backpropagation, 
